...
{
    [Serializable]
    public class Increment_Period : IDialog<object>
    {
        ...
        private async Task abc(IDialogContext context, IAwaitable<object> result)
        {
            ...
            if (token != null)
            {
                ...
                var probation_period_response = await probation_period.ProbationPeroid(token, employee_id);
                await context.PostAsync($"Your Join Date is {probation_period_response.ResponseJSON.incrementperiod} ");
                context.Done(true);
            }
        }
