...
{
    [Serializable]
    public class Absent : IDialog<object>
    {
        ...
        private async Task Halfdays(IDialogContext context, IAwaitable<object> result)
        {
            ...
            try
            {
                ...
                {
                    await context.PostAsync($"Your absent in payroll month {DimattendancemonthlyDetails.ResponseJSON.PayrollMonth}" +
                    $" are {DimattendancemonthlyDetails.ResponseJSON.Absent} ");
                    context.Done(true);
                }
                else
                {
