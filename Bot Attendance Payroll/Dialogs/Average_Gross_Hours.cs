...
{
    [Serializable]
    public class Average_Gross_Hours : IDialog<object>
    {
        ...
        private async Task Halfdays(IDialogContext context, IAwaitable<object> result)
        {
            try
            {
                ...
                {
                    await context.PostAsync($"Your average gross hours in payroll month {DimattendancemonthlyDetails.ResponseJSON.PayrollMonth}" +
                    $" are {DimattendancemonthlyDetails.ResponseJSON.AvgGrossHours} ");
