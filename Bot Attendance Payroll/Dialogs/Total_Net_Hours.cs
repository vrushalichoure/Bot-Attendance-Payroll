...
{
    [Serializable]
    public class Total_Net_Hours : IDialog<object>
    {
        ...
        private async Task Halfdays(IDialogContext context, IAwaitable<object> result)
        {
            try
            {
                ...
                {
                    await context.PostAsync($"Your total net hours in payroll month {DimattendancemonthlyDetails.ResponseJSON.PayrollMonth}" +
                    $" are {DimattendancemonthlyDetails.ResponseJSON.TotalNetHours} ");
