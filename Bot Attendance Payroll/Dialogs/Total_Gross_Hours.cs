...
{
    [Serializable]
    public class Total_Gross_Hours : IDialog<object>
    {
        ...
        private async Task Halfdays(IDialogContext context, IAwaitable<object> result)
        {
            ...
            year = activity.Text;
                ...
                var Dimattendancemonthly = new DimattendancemonthlyClient();
                    ...
                    {
                        await context.PostAsync($"Your total gross hours in payroll month {DimattendancemonthlyDetails.ResponseJSON.PayrollMonth}" +
                        $" are {DimattendancemonthlyDetails.ResponseJSON.TotalGrossHours} ");
                        context.Done(true);
                    }
                    else
                    {
